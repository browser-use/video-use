...
First-time install lives in `install.md` (`uv tool install video-use`, ffmpeg, skill registration, API key). Don't re-run it every session; on cold start just run `video-use doctor` — it verifies everything below in one shot:

- `video-use` on PATH. If it's missing but this SKILL.md sits inside a git clone, fall back to `python helpers/<name>.py` relative to this file — the two forms take identical arguments.
- `ELEVENLABS_API_KEY` resolves — environment, `./.env`, or `~/.config/video-use/.env`. If missing, ask the user to paste one and store it with `video-use key` (never write it to the user's `<videos_dir>`).
- `ffmpeg` + `ffprobe` on PATH.
- Node.js + npm available if the session needs HyperFrames or Remotion slots. HyperFrames currently requires Node.js 22+.
- `yt-dlp`, HyperFrames, Remotion, Manim installed only on first use.
- First-use animation setup happens inside the slot directory, never at the video-use install location. HyperFrames can be invoked with `npx --yes hyperframes ...`; Remotion can be scaffolded with `npx create-video@latest` or installed as a project-local dependency before using its `remotion render` command.
- This skill bundles a `manim-video` sub-skill. Read it via `video-use skill manim-video` (or at `$(video-use where)/skills/manim-video/`) when building a Manim slot.

## Helpers

- **`video-use transcribe <video>`** — single-file Scribe call. `--num-speakers N` optional. Cached.
- **`video-use transcribe-batch <videos_dir>`** — 4-worker parallel transcription. Use for multi-take.
- **`video-use pack --edit-dir <dir>`** — `transcripts/*.json` → `takes_packed.md` (phrase-level, break on silence ≥ 0.5s).
- **`video-use timeline <video> <start> <end>`** — filmstrip + waveform PNG. On-demand visual drill-down. **Not a scan tool** — use it at decision points, not constantly.
- **`video-use render <edl.json> -o <out>`** — per-segment extract → concat → overlays (PTS-shifted) → subtitles LAST. `--preview` for 720p fast. `--build-subtitles` to generate master.srt inline.
- **`video-use grade <in> -o <out>`** — ffmpeg filter chain grade. Presets + `--filter '<raw>'` for custom.

For animations, create `<edit>/animations/slot_<id>/` with `Bash` and spawn a sub-agent via the `Agent` tool.

## The process

1. **Inventory.** `ffprobe` every source. `video-use transcribe-batch` on the directory. `video-use pack` to produce `takes_packed.md`. Sample one or two `video-use timeline` views for a visual first impression.
...
5. **Execute.** Produce `edl.json` via the editor sub-agent brief. Drill into `video-use timeline` at ambiguous moments. Build animations in parallel sub-agents. Apply grade per-segment. Compose via `video-use render`.
6. **Preview.** `video-use render --preview`.
7. **Self-eval (before showing the user).** Run `video-use timeline` on the **rendered output** (not the sources) at every cut boundary (±1.5s window). Check each image for:
...
`video-use pack` reads all `transcripts/*.json` and produces one markdown file where each take is a list of phrase-level lines, each prefixed with its `[start-end]` time range. Phrases break on any silence ≥ 0.5s OR speaker change. This is the artifact the editor sub-agent reads to pick cuts — it gives word-boundary precision from text alone at 1/10 the tokens of raw JSON.
...
Your job is to **reason about the image**, not apply a preset. Look at a frame (via `video-use timeline`), decide what's wrong, adjust one thing, look again.
...
**Example filter chains** (`video-use grade --list-presets` shows them; use them as starting points or mix your own):
...
For anything else — portraiture, nature, product, music video, documentary — invent your own chain. `video-use grade --filter '<raw ffmpeg>'` accepts any filter string.
...
**`bold-overlay`** — short-form tech launch, fast-paced social. 2-word chunks, UPPERCASE, break on punctuation, Helvetica 18 Bold, white-on-outline, `MarginV=35`. `video-use render` ships with this as `SUB_FORCE_STYLE`.
...
- **Manim** — formal diagrams, state machines, equation derivations, graph morphs. Read the bundled manim-video skill (`video-use skill manim-video`; references at `$(video-use where)/skills/manim-video/references/`) for depth.
...
Match the source unless the user asked for something specific. Common targets: `1920×1080@24` cinematic, `1920×1080@30` screen content, `1080×1920@30` vertical social, `3840×2160@24` 4K cinema, `1080×1080@30` square. `video-use render` defaults the scale to 1080p from any source; pass `--filter` or edit the extract command for other targets. Worth asking the user which delivery format matters.
